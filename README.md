...
For `alembic revision --autogenerate` to detect model changes, `alembic/env.py` needs `target_metadata` pointed at your models' `Base.metadata` (it defaults to `None`):
